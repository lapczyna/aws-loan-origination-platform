...
interface DocumentOutboxJpaRepository extends JpaRepository<DocumentOutboxEventEntity, UUID> {
    ...
    @Query(
            value =
                    """
                    SELECT * FROM document.outbox_event
                    ...
                    """,
            nativeQuery = true)
    List<DocumentOutboxEventEntity> claimBatch(@Param("now") OffsetDateTime now, @Param("batchSize") int batchSize);


    long countByStatus(String status);

    /** Backs the "oldest unpublished outbox record" gauge and its alarm. */
    @Query("select min(e.createdAt) from DocumentOutboxEventEntity e where e.status <> 'PUBLISHED'")
    Instant oldestUnpublishedCreatedAt();

    List<DocumentOutboxEventEntity> findByAggregateIdOrderByCreatedAtAsc(String aggregateId);
}
