 ...
 *
 * <p>Holds the complete serialised envelope, exactly as it will be published. The
 * publisher does not re-render the event: whatever was decided at commit time is
 * what reaches Kafka, so a later change to the mapping code cannot retroactively
 * alter an event that is already waiting to be sent.
 */
@Entity
@Table(name = "outbox_event", schema = "document")
class DocumentOutboxEventEntity {
    ...
    protected DocumentOutboxEventEntity() {}

    @SuppressWarnings("checkstyle:ParameterNumber")
    DocumentOutboxEventEntity(
            UUID id,
    ...
    public String toString() {
        return "DocumentOutboxEventEntity[id=" + id + ", type=" + eventType + ", status=" + status + ", attempts=" + attempts
                + "]";
    }
}
