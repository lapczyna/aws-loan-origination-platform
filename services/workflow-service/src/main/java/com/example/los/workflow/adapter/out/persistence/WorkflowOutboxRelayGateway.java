 ...
 */
@Component
public class WorkflowOutboxRelayGateway {
    ...
    WorkflowOutboxRelayGateway(WorkflowOutboxJpaRepository outbox, Clock clock) {
        this.outbox = outbox;
        this.clock = clock;
    }
    ...
    public long pendingCount() {
        return outbox.countByStatus(WorkflowOutboxEventEntity.Status.PENDING.name());
    }

    /** Backs the dead-letter alarm: events that exhausted their retry budget. */
    @Transactional(readOnly = true)
    public long permanentlyFailedCount() {
        return outbox.countByStatus(WorkflowOutboxEventEntity.Status.FAILED.name());
    }
