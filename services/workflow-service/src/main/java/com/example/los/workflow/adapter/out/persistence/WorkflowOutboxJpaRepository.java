...
interface WorkflowOutboxJpaRepository extends JpaRepository<WorkflowOutboxEventEntity, UUID> {
    ...
    @Query(
            value =
                    """
                    SELECT * FROM workflow.outbox_event
                    ...
                    """,
            nativeQuery = true)
    List<WorkflowOutboxEventEntity> claimBatch(@Param("now") OffsetDateTime now, @Param("batchSize") int batchSize);


    long countByStatus(String status);

    /** Backs the "oldest unpublished outbox record" gauge and its alarm. */
    @Query("select min(e.createdAt) from WorkflowOutboxEventEntity e where e.status <> 'PUBLISHED'")
    Instant oldestUnpublishedCreatedAt();

    List<WorkflowOutboxEventEntity> findByAggregateIdOrderByCreatedAtAsc(String aggregateId);
}
